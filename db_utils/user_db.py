...
class UserDB:
    ...
    def add_or_update_user(
        self, sender_id: str, umo_of_bot: str, is_admin: bool = False
    ) -> None:
        """新增或更新用户信息。
        ...
        with self.dbm.get_conn() as conn:
            conn.execute(
                """
                INSERT INTO Users (sender_id, umo_of_bot, is_admin) VALUES (?, ?, ?)
                ON CONFLICT(sender_id) DO UPDATE SET
                    umo_of_bot = excluded.umo_of_bot,
                    is_admin = excluded.is_admin
                """,
                (sender_id, umo_of_bot, int(is_admin)),
            )
    ...
    def get_user(self, sender_id: str) -> User | None:
        ...
        with self.dbm.get_conn() as conn:
            row = conn.execute(
                "SELECT sender_id, umo_of_bot, is_admin FROM Users WHERE sender_id = ?",
                (sender_id,),
            ).fetchone()
        if row is None:
            return None
    ...
    def get_users_by_tag(self, tag: str) -> List[User]:
        ...
        with self.dbm.get_conn() as conn:
            ...
            cursor.execute(
                """
                SELECT u.sender_id, u.umo_of_bot, u.is_admin
                FROM Users as u
