...
[![Version](https://img.shields.io/badge/Version-v3.1.0-blue)](https://github.com/LinearBall/astrbot_plugin_list_reminder/releases)

</div>

## 目录

- [AstrBot 列表提醒插件 v3.1](#astrbot-列表提醒插件-v31)
  - [目录](#目录)
  - [1. 功能特点](#1-功能特点)
  - [2. 安装方法](#2-安装方法)
  - [3. 第一次使用](#3-第一次使用)
...
| 功能                 | 描述                                                                     |
| -------------------- | ------------------------------------------------------------------------ |
| AI 智能识别          | 自动识别创建提醒、按标签批量删除等意图，符合条件时自动执行               |
| 自然语言时间解析     | 支持「明天」「后天」「下周一」「X小时后」等相对时间，自动换算为绝对时间 |
| 定时提醒             | 到达设定时间后自动在对应聊天窗口发送提醒消息                             |
| 任务管理命令         | 通过命令快捷查看任务列表、清空任务                                       |
| 任务标签             | 创建任务时可附带标签，列表、即时回复与提醒消息中都会显示标签；支持按标签批量删除任务 |
| 按用户标签群发任务   | 提醒带有某个用户标签（如部门/群组）的所有人，自动为每人分别创建任务     |
...
| 发送「提醒工程一部的人这周四上午9点有个讨论会，标签 会议」 | 按用户标签批量创建任务 |
| 发送「删除所有有会议标签的任务」      | 按任务标签批量删除当前用户的任务 |
...
> - 按用户标签群发时，会为每个带有该标签的用户分别创建任务，并通过各自的私聊会话推送提醒。
> - 按标签删除任务仅删除当前发送者自己名下带有该标签的任务，不影响其他用户。
...
1. 自然语言任务管理升级：意图识别区分「创建任务」与「按任务标签批量删除任务」，无关消息不再触发
2. 支持按用户标签批量创建任务：例如「提醒工程一部的人这周四上午9点有个讨论会，标签 会议」，会为所有带「工程一部」标签的用户分别创建任务，并通过各自的私聊会话推送提醒
3. 支持按任务标签批量删除任务：例如「删除所有有会议标签的任务」，删除当前用户下带有该标签的全部任务；没有匹配任务时返回错误提示
4. 修复 `/列表提醒` 系列命令（包括以引用/回复形式触发时）被误识别为自然语言而创建提醒任务的问题
